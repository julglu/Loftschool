...
function collectDOMStat(root) {
    ...
    function _collectDOMStat(root) {

        for (let child of root.children) {

            result.tags[child.nodeName] = (result.tags[child.nodeName] || 0) + 1;

            for (let i = 0; i < child.classList.length; i++) {
                result.classes[child.classList[i]] = (result.classes[child.classList[i]] || 0) + 1;
            }

            if (child.nodeType === 3) {
                result.texts += 1;
            }
            _collectDOMStat(child);
        }

        result.texts = 3;
    }

    _collectDOMStat(root);
...
function observeChildNodes(where, fn) {
    ...
    var observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            ...
            let nodes = [];
            let addedOrRemovedNodes;

            if (mutation.addedNodes.length > 0) {
                addedOrRemovedNodes = mutation.addedNodes;
            } else if (mutation.removedNodes.length > 0) {
                addedOrRemovedNodes = mutation.removedNodes;
            }

            for (let i = 0; i < addedOrRemovedNodes.length; i++) {
                nodes.push(addedOrRemovedNodes[i]);
            }
